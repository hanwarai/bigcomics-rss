...
def test_filters_paid_episodes(requests_mock: rm_module.Mocker, feeds_dir: Path) -> None:
    html = _series_html(
        title="らーめん再遊記",
        episodes=[
...
def test_filters_waitfree_episodes(requests_mock: rm_module.Mocker, feeds_dir: Path) -> None:
...
def test_returns_none_without_title(requests_mock: rm_module.Mocker, feeds_dir: Path) -> None:
